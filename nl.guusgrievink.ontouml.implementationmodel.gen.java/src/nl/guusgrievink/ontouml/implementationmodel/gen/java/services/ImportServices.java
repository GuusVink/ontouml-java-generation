...
public class ImportServices {
	...
	public String reqImport(Classifier aClassifier) {
		...
		if (qualifiedName != null) {
			...
			if (lastIndexOf == -1) {
				...
				for (String sortedImportedType : sortedImportedTypes) {
					// Fixed bug Guus 25/09/2024: Previously the +1 on the length missed. Resulting in imports never being removed here
					if (sortedImportedType.startsWith(packageName)
							&& sortedImportedType.indexOf('.', packageName.length() + 1) == -1) {
						importedTypesToRemove.add(sortedImportedType);
					}
				}
			}
	...
	private Set<String> typeToImports(Classifier aClassifier) {
		...
		AncestorServices ancestorServices = new AncestorServices();
		
		// Add imports for methods that will be included due to interface inheritance
		//	check if an interface is implemented
		if (aClassifier instanceof Class aClass && aClass.getInterfaceRealizations().size() > 0) {
			ArrayList<Interface> implementedInterfaces = new ArrayList<Interface>();
			
			// Record all interfaces that are implemented, including super interfaces that are extended by said interfaces
			for (Interface directlyImplementedInterface : aClass.getInterfaceRealizations().stream().map(ir -> ir.getContract()).toList()) {
				implementedInterfaces.add(directlyImplementedInterface);
				implementedInterfaces.addAll(ancestorServices.getAllInterfaceAncestors(directlyImplementedInterface));
			}
			
			// Add all types that are required for this interface implementations
			for (Interface implementedInterface : implementedInterfaces) {
				importedTypes.addAll(typeToImports(implementedInterface));
			}
		}
		

		// Changed Guus, 25/09/2024
		// Import from attributes
		Set<Property> attributes = new HashSet<>(aClassifier.getAttributes()); // deep copy as we will add elements
		
		
		// Add attributes from navigable association ends
		for (Association association : aClassifier.getAssociations()) {
			for (Property navigable : association.getNavigableOwnedEnds()) {
				if (! navigable.getType().equals((Type) aClassifier)) {
					attributes.add(navigable);
				}
			}
		}
